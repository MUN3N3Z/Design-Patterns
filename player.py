...
# Pick worker strategy interface
class Strategy():
...
# Pick move strategy interface
class RandomStrategy(Strategy):
    def __init__(self, player, board) -> None:
        super().__init__(player, board)
    def pick_worker(self):
        while True:
            random_worker = self._player._workers[random.randint(0, 1)]
    ...
    def pick_move(self):
        while True:
            ...
            if self._board.check_move(self._chosen_worker, random_direction, False):
                self.score = self._player.calculate_stats(self._chosen_worker, random_direction)
...
class HeuristicStrategy(Strategy):
    ...
    def pick_worker(self):
        ...
        for worker in range(0, 2):
            new_worker_name = self._player._workers[worker].name
            
            for direction in range(0, 8):
                new_direction = Board.valid_directions[direction]
                #  Translate new 2-d proposed position to 1-d
                height_score, center_score, distance_score = self._player.calculate_stats(new_worker_name, new_direction)
        ...
        self._chosen_worker = self._player._workers[worker].name
...
class HumanStrategy(Strategy):
    ...
    def pick_worker(self):
        while True:
            ...
            elif piece not in self._player.worker_names:
                print("That is not your worker")
            elif not self._player._workers[index].has_valid_moves:
    ...
    def pick_move(self):
        while True:
            ...
            if move not in Board.valid_directions:
                ...
                break
        self.score = self._player.calculate_stats(self._chosen_worker, move)
        return move
    ...
    def pick_build(self):
        while True:
            ...
            elif not self._board.check_build(self._chosen_worker, build, False):
...
class Player:
    def __init__(self, name:str, board: Board, strategy: str) -> None:
        ...
        if strategy == "human":
            self._strategy = HumanStrategy(self, self._board)
        elif strategy == "heuristic":
            self._strategy = HeuristicStrategy(self, self._board)
        elif strategy == "random":
            self._strategy = RandomStrategy(self, self._board)
        self.pick_worker = self._strategy.pick_worker
        self.pick_move = self._strategy.pick_move
        self.pick_build = self._strategy.pick_build
    ...
    def board(self, new_board):
        ...
        self._strategy._board = new_board
